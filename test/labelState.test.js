...
test('draft MR is skipped even if labeled', () => {
  ...
  const result = deriveMrState(mr, { now: NOW, noLabelGraceMs: 900000 });
...
test('MRs involving neither vibecoder are still tracked (no allowlist filter)', () => {
  const mr = baseMr({ authorUsername: 'carol', reviewerUsername: 'dave', labels: [LABELS.DONE] });
  const result = deriveMrState(mr, { now: NOW, noLabelGraceMs: 900000 });
  assert.deepEqual(result, { state: STATES.DONE, responsibleUsername: 'dave' });
});
